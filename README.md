...
| 模式 | 说明 | 快捷键 |
|---|---|---|
| **悬浮窗模式** | 透明洞口常驻置顶,洞内即捕获区;可透看后方内容,一键识别 | `Ctrl+1` |
| **自由截图模式** | 全屏半透明遮罩,拖拽框选任意区域;可「立即识别」或「设为悬浮窗区域」 | `Ctrl+Shift+A` / `Ctrl+3` |
| **迷你条模式** | 极简工具条不遮挡画面,点击即进入框选识别 | `Ctrl+2` |
...
- **全局主题 + 自定义主题**:浅色 / 深色 / 灰色 / 护眼 / 高对比 5 套预设,支持逐项配色、圆角、字号、面板不透明度、洞口边框样式;主题可导出/导入 JSON,所有窗口实时同步
